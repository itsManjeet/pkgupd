...
std::optional<InstalledMetaInfo> SystemDatabase::get(const std::string &id) const {
...
void SystemDatabase::remove(const InstalledMetaInfo &installed_meta_info) {
    auto const data_file = std::filesystem::path(data_dir) / installed_meta_info.name();
    ...
    if (auto const iter = mPackages.find(installed_meta_info.id); iter == mPackages.end()) {
