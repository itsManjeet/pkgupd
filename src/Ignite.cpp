...
Ignite::Ignite(Configuration& config, std::filesystem::path project_path,
        ...
        : config{config}, project_path(std::move(project_path)),
          cache_path(cache_path.empty() ? this->project_path / "cache"
                                        : std::move(cache_path)) {
...
void Ignite::load() {
    ...
    for (auto const& i : std::filesystem::recursive_directory_iterator(
        ...
        if (i.is_regular_file() && i.path().has_extension() &&
            ...
            auto element_path = std::filesystem::relative(
                    i.path(), project_path / "elements");
            try {
                pool[element_path.string()] =
                        Builder::BuildInfo(i.path(), project_path);
...
std::string Ignite::hash(const Builder::BuildInfo& build_info) {
    ...
    for (auto const& d :
            {build_info.depends, build_info.build_time_depends, includes}) {
        for (auto const& i : d) {
            {
...
void Ignite::build(const Builder::BuildInfo& build_info, Engine* engine) {
    ...
    auto build_root =
            ...
            build_info.config.get<std::string>("build-dir", subdir->string());
...
Container Ignite::setup_container(const Builder::BuildInfo& build_info,
        Engine* engine, const ContainerType container_type) {
    ...
    auto host_root = (cache_path / "temp" /
    ...
    if (container_type == ContainerType::Build) {
        depends.insert(depends.end(), build_info.build_time_depends.begin(),
    ...
    if (build_info.config.node["include"]) {
        ...
        resolve(include, states, false,
                build_info.config.get<bool>("include-depends", true), false);
        ...
        for (auto const& [path, info, cached] : states) {
            ...
            installation_path = build_info.config.get<std::string>(
