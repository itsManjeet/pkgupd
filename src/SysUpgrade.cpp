...
std::optional<SysUpgrade::UpdateInfo> SysUpgrade::update(
        const std::string& url, bool dry_run) {
    auto response = Http().url(url + "/update").get();
    if (!response.contains("release")) {
        throw std::runtime_error("invalid response");
    }
    auto update_info = UpdateInfo{
            .version = response["release"].get<int>(),
    ...
    auto temp_file =
            sysroot->deployment_dir() / std::to_string(update_info.version);
    auto installed_version = INT_MIN;
    if (!sysroot->deployments().empty()) {
        installed_version = sysroot->deployments().front().version();
    }
    if (installed_version >= update_info.version) { return std::nullopt; }
    if (dry_run) return update_info;

    if (sysroot->deployments().empty()) {
        if (!std::filesystem::exists(temp_file)) {
            Http().url(update_info.url).download(temp_file);
        }

    } else {
        Executor("/bin/zsync")
                .arg("-i")
                .arg(sysroot->deployments().front().path())
                .arg("-o")
                .arg(temp_file)
