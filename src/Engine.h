...
public:
    ...
    [[nodiscard]] std::map<std::string, InstalledMetaInfo> const&
    list_installed() const;

    void sync(bool force = false);

    void resolve(const std::vector<std::string>& ids,
            std::vector<MetaInfo>& output_meta_infos) const;

    void resolve(const std::vector<MetaInfo>& ids,
            std::vector<MetaInfo>& output_meta_infos) const;

    void list_installed_files(const InstalledMetaInfo& installed_meta_info,
            std::vector<std::string>& files_list);

    [[nodiscard]] std::filesystem::path download(
