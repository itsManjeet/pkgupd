...
PKGUPD_UNLOCKED_MODULE_HELP(cleanup) {
...
PKGUPD_UNLOCKED_MODULE(cleanup) {
    auto pkg_dir = config->get<std::string>(DIR_CACHE, DEFAULT_CACHE_DIR) +
                   "/packages";
    auto src_dir =
            config->get<std::string>(DIR_CACHE, DEFAULT_CACHE_DIR) + "/sources";
    if (std::filesystem::exists(pkg_dir) &&
        ...
        if (error) { ERROR("failed to remove '" + pkg_dir + "'"); }
