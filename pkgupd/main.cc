...
main (int argc, char **argv)
{
  ...
  vector<string> Mirrors
      = { "https://rlxos.cloudtb.online", "https://apps.rlxos.dev" };
              ...
              for (auto const &i : config["EnvironmentVariables"])
                {
                  ...
                  DEBUG ("setting " << ev);
                  setenv (
                      ev.substr (0, idx).c_str (),
                      ev.substr (idx + 1, ev.length () - (idx + 1)).c_str (),
                      1);
                }
            }
        }
  ...
  libpkgupd::Pkgupd pkgupd (SystemDatabase, CachePath, Mirrors, Version,
                            RootDir, hasFlag (Flag::Force),
  ...
  auto doTask = [&] (Task task, libpkgupd::Pkgupd &pkgupd,
    ...
    auto check_exact = [&] (int count) {
      if (args.size () != count)
        {
          throw runtime_error ("need '" + to_string (count)
                               + "' arguments but "
                               + to_string (args.size ()));
        }
    };
              ...
              for (auto const &i : args)
                {
                  if (std::filesystem::exists (i)
                  ...
                  else
                    {
                      ...
                      dependencies.insert (dependencies.end (), dep.begin (),
                                           dep.end ());
                    }
                }
