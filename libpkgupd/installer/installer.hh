...
class Installer : public Object {
 public:
  class Injector : public Object {
   ...
   public:
    ...
    virtual std::shared_ptr<PackageInfo> inject(
        char const* path, std::vector<std::string>& files) = 0;
 ...
 public:
  ...
  bool install(std::vector<std::string> pkgs, SystemDatabase* sys_db);
  bool install(std::vector<std::shared_ptr<PackageInfo>> pkgs,
               Repository* repository, SystemDatabase* sys_db);
};
}  // namespace rlxos::libpkgupd
