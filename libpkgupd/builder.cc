...
#undef X
    ...
    bool Builder::build(Recipe *recipe, SystemDatabase *systemDatabase,
        ...
        if (mConfig->get("build.install", false)) {
            auto installer = Installer(mConfig);
            std::vector<std::pair<std::string, PackageInfo *>> installData;
            for(int i = 0; i < mPackages.size(); i++) {
                installData.push_back({mPackages[i], packagesdir[i].first.get()});
            }
    ...
    bool Builder::pack(
        ...
        for (auto const &i: dirs) {
            ...
            if (mConfig->get<bool>("build.env", true)) {
                ...
                auto resolver = Resolver<PackageInfo *>(
